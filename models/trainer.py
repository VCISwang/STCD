    ...
    def train_models(self):
        ...
        for self.epoch_id in range(self.epoch_to_start, self.max_num_epochs):
            ...
            for self.batch_id, batch in tqdm(enumerate(self.dataloaders['train'], 0), total=total):
                ...
                self._timer_update()
            if self.max_num_epochs == 100:
                if self.epoch_id > 50 and (self.epoch_id + 1) % 10 == 0:
                    save_epoch_path = os.path.join(self.args.weight_dir, str(self.epoch_id)+".pth")
                    torch.save(self.net_G, save_epoch_path)

            elif self.max_num_epochs == 200:
                if self.epoch_id > 100 and (self.epoch_id + 1) % 20 == 0:
                    save_epoch_path = os.path.join(self.args.weight_dir, str(self.epoch_id)+".pth")
                    torch.save(self.net_G, save_epoch_path)
